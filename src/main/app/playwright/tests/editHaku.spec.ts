...
test.describe('Edit haku', () => {
  ...
  // Kielivälilehdet. Kielipäätteinen kenttä poistuu rekisteristä aina kun
  ...
  // Piilotetun kentän tyhjennys Haku-footerille. Kolmella muulla rungon rakentavalla
  // footerilla on jo testi, jossa piilotettu kenttä tyhjennetään; Haulta se puuttui.
  //
  ...
  // Tallentamattomien muutosten hylkääminen ja paluu. HUOM: tämä EI havaitse
  // rekisterin vuotoa lomakkeiden VÄLILLÄ: samalle lomakkeelle palattaessa kentät
  // mounttaavat uudelleen ja poistuvat poistuneiden joukosta, eli vuoto paikkaa
  // itsensä. Sen havaitseminen vaatisi siirtymän toisen entiteetin lomakkeelle.
  // Nimetty sen mukaan mitä testi todella tekee.
  //
  // Testaa VASTAKKAISTA virhesuuntaa kuin edellinen testi: ei sitä jääkö
  // tyhjentäminen tekemättä, vaan sitä tyhjennetäänkö
  ...
  // Taustahaku tallennuksen jälkeen ei saa muuttaa sitä, mitä seuraava
  // tallennus lähettää.
  //
  // Onnistunut tallennus kutsuu afterUpdatea, joka invalidoi kyselyn
  // (utils/afterUpdate.ts). QueryResultWrapper menee silloin isFetching-tilaan
  ...
  test('Should send an emptied translated field as an empty string', async ({
    ...
    // käännetyn kentän muotoon {} (isEmptyTranslatedField utils/index.ts:ssä), eli
