...
// Klikataan radion LABELIA, ei pelkkää toimipisteen nimeä: getByText(nimi) saattoi
// osua muuhun kuin labeliin, jolloin klikkaus meni tyhjään ja jarjestyspaikkaOid
// puuttui rungosta - tallennus lähti silti.
...
test.describe('Edit hakukohde', () => {
  ...
  // testit ajavat sen koodin, joka tyhjentää dataa backendistä.
  test('should clear the hakukohde hakuajat when the shared haku schedule is taken into use', ({
    ...
    mutationTest({ page, testInfo }, async () => {
      ...
      // eriHakuaika pois -> koko hakuajat.hakuajat-FieldArray katoaa. Vanhempi ja
      // lapset poistuvat rekisteristä samalla kertaa, ja getValuesForSavingin
      // kirjoitusjärjestys ratkaisee lähteekö backendiin null vai lista nulleja.
  ...
  test('should clear the per-liite toimitusaika when a shared one is taken into use', ({
    ...
    mutationTest({ page, testInfo }, async () => {
      ...
      await withinSection(page, 'liitteet', async section => {
        ...
        // NÄKYVYYS, ei vain runko: jos useFieldValue-luku (LiitteetFields.tsx)
  ...
  // näppäinpainallus muuttaa taulukon arvoa ja renderöi FieldArrayn.
