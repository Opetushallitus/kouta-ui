...
export class KoulutusSelector extends Component {
  ...
  matchOption = (option) => option.metadata[0]['nimi'].indexOf(this.state.filter) > -1;

  compareOptions = (a, b) => {
    const aName =  a.metadata[0]['nimi'].toLowerCase();
    const bName = b.metadata[0]['nimi'].toLowerCase();
