...
const SessionErrorModal = ({sessionError, setSessionError, apiUrls, t}) => {
  return <Modal open={sessionError}>
    <ModalHeader>
    {sessionError === RESPONSE_ERROR_UNAUTHORIZED ? t('ilmoitukset.istuntoVirhe.otsikko') : t('ilmoitukset.yhteysVirhe.otsikko')}
    </ModalHeader>
    <ModalBody>
    {sessionError === RESPONSE_ERROR_UNAUTHORIZED ? t('ilmoitukset.istuntoVirhe.viesti') : t('ilmoitukset.yhteysVirhe.viesti')}
    </ModalBody>
    <ModalFooter>
      <Flex justifyEnd>
        <MyButton onClick={() => isDev ? window.open(apiUrls.url('cas.login')) : window.location.replace(apiUrls.url('cas.login'))}>
          {t('yleiset.meneLoginSivulle')}
        </MyButton>
        <MyButton marginLeft={1} color="danger" onClick={() => setSessionError(null)}>
