...
//
// formNameProp: hookit ottavat sen yhteensopivuuden vuoksi ja jättävät sen huomiotta.
// Se on redux-formin globaalin storen käsite; react-final-formissa tila asuu
// lomakkeen sisällä, ja yksikään kutsupaikka ei anna toisen lomakkeen nimeä.
...
// Lomaketila footereille: alkuarvot ja kenttärekisterin joukot.
...
// Vain initialValues tilataan. Arvoja ei tilata, koska yksikään kutsuja ei lue niitä:
// tallennus saa arvot kirjaston onSubmitista. Values-tilaus renderöisi jokaisen
// footerin joka näppäinpainalluksella.
//
// Paluuarvo muistetaan, jotta footerien useCallback-riippuvuus form pysyy vakaana:
// uusi olio joka renderillä tekisi tallennuskäsittelijästä uuden ja ajaisi
// useRegisterSubmitHandlerin efektin joka renderillä.
export const useForm = (): FormStateWithCorrectTypes => {
  ...
  const form = useMemo(
    () => ({
      ...
      // Getterit, EIVÄT tilannekuvia. Rekisteri elää refeissä eikä
      // FieldRegistryProvider renderöi uudelleen kenttien mountatessa - se on
      // tarkoituksellista, koska muuten kielivälilehden vaihto aiheuttaisi satoja
      // uudelleenrenderöintejä. Renderin aikana luettu arvo olisi siis helposti tyhjä
      // tai vanhentunut: footer, joka pitää kiinni const form = useForm():sta, lukisi
      // joukon ennen kuin kentät ehtivät rekisteröityä. Getterinä se evaluoituu vasta
      // lukuhetkellä, joka on tallennushetki.
      //
      // Samasta syystä footerit antavat useCallbackin riippuvuuslistaan formin, EIVÄT
...
// Vastaa redux-formin initializea: asettaa uuden lähtötilan, jolloin dirty nollautuu
// eikä "tallentamattomia muutoksia" -varoitus jää päälle. Tallennuksen aloitus ja
// lopetus tulevat kirjastolta (form.submit asettaa submittingin, onSubmitin
// paluuarvo submitErrorsin), joten elinkaaresta jää omaksi vain tämä.
export const useReinitialize = () => {
  const form = useRffForm();
  return useCallback((values: any) => form.initialize(values), [form]);
};

// Tallennuksen laukaisu. Kirjasto ohittaa kutsun jos tallennus on jo kesken
// (final-form: submit() palaa heti jos formState.submitting), joten tuplaklikkaus
// ei vaadi erillistä disablointia.
export function useSubmitForm(): () => void {
  const form = useRffForm();
  return useCallback(() => {
    form.submit();
  }, [form]);
...
// Tallennuskäsittelijän rekisteröinti. useSaveForm antaa tänne oman funktionsa, ja
// ReactFinalFormin <Form> kutsuu sitä onSubmitissa. Efektissä, jottei renderin
// aikainen kirjoitus jää keskeneräisestä renderistä.
export function useRegisterSubmitHandler(handler: SubmitHandler): void {
  const ref = useSubmitHandlerRef();
  useEffect(() => {
    ref.current = handler;
    return () => {
...
// Palauttaa vain changen. Aiemmin tämä sitoi redux-formin KOKO action creator -pinnan
// lomakkeeseen, mutta kutsupaikat käyttävät yksinomaan changea. Kapea rajapinta on
// myös se, mikä tekee kirjastonvaihdosta mahdollisen: react-final-formissa ei ole
// vastaavaa action creator -pintaa.
...
export function useIsSubmitting(): boolean {
  return Boolean(
    useFormState({ subscription: { submitting: true } }).submitting
  );
}

// Tosi vain epäonnistuneen tallennusyrityksen jälkeen, kuten redux-formin
// submitFailed: kirjasto nollaa sen form.submit():n alussa ja asettaa, jos onSubmit
// palauttaa virheitä.
export function useSubmitFailed(): boolean {
  return Boolean(
    useFormState({ subscription: { submitFailed: true } }).submitFailed
  );
}

export function useSubmitErrors<TErrors = Record<string, any>>(): TErrors {
  return useFormState({ subscription: { submitErrors: true } })
    .submitErrors as TErrors;
}

// RAAKA arvo, ei muotoiltu. react-final-formin oletus-format muuttaa undefinedin
// tyhjäksi merkkijonoksi, jolloin lukija ei erota "ei arvoa" tyhjästä arvosta.
//
 ...
 *
 * PERFORMANCE: Each field gets its own subscription, so components only re-render
 * when the specific fields they use change (not on every form change).
 *
